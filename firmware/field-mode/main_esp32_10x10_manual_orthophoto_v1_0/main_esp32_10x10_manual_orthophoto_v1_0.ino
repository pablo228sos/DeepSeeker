...
void configureActiveLegStopTimes(uint32_t nowMs);
void beginStraightLeg(const GeoPoint& start, const GeoPoint& finish, bool outbound);
void beginFarTurn();
...
// Photo-first manual sweep state must be declared before camera response handling.
bool manualPhotoSweepEnabled = false;
uint32_t manualPhotoLastRequestMs = 0;
float manualPhotoDistanceSinceLastM = 0.0f;
uint32_t manualPhotoSchedulerLastMs = 0;
char manualDriveCommand = '0';

// Dense orthophoto scheduler. The target spacing is deliberately much smaller
// than a normal GPS fix. Along-track GPS progress is used when available, while
// a time deadline guarantees that water motion or GPS jitter cannot create gaps.
constexpr float ORTHO_TARGET_PHOTO_STEP_M = 0.20f;
constexpr uint32_t ORTHO_SOFT_PHOTO_DEADLINE_MS = 850;
constexpr uint32_t ORTHO_HARD_PHOTO_DEADLINE_MS = 1250;
float photoAlongAtLastRequestM = -1000.0f;
uint32_t laneExpectedPhotoCount = 0;
uint32_t laneSavedPhotoCount = 0;
bool lanePhotoCoverageGood = false;

uint8_t consecutiveCaptureFailures = 0;
constexpr uint8_t MAX_CONSECUTIVE_CAPTURE_FAILURES = 5;
...
void processCameraResponse(String response) {
  ...
  if (response.startsWith("CAPTURE_PARTIAL,")) {
    ...
    lastSystemEvent = "CAPTURE_PARTIAL_IMAGE_SAVED";
...
// ============================================================
// 10x10 MANUAL ORTHOPHOTO V1.0 — dense one-way strips + manual fallback
...
// 10 x 10 m shore-shuttle geometry.
// Save only two shore corners:
//   A = left end of the shore edge
//   D = right end of the shore edge
// Stand at A and look toward D; choose whether the water is LEFT or RIGHT.
// B and C are generated automatically from the known 10 m square.
GeoPoint surveyA;
GeoPoint surveyB;
GeoPoint surveyC;
GeoPoint surveyD;
...
int8_t waterSideSign = 1;  // +1 = left of A->D, -1 = right of A->D

constexpr float SHUTTLE_SQUARE_SIZE_M = 10.0f;
constexpr float SHUTTLE_SIDE_MARGIN_M = 0.35f;
constexpr float SHUTTLE_SHORE_MARGIN_M = 0.30f;
constexpr float SHUTTLE_FAR_MARGIN_M = 0.30f;
constexpr uint8_t MAX_SURVEY_LANES = 20;
...
float surveyRequestedSpacingM = 0.70f;
float surveyActualSpacingM = 0.0f;
float surveyLengthM = SHUTTLE_SQUARE_SIZE_M;
float surveyWidthM = SHUTTLE_SQUARE_SIZE_M;
float surveyAreaM2 = SHUTTLE_SQUARE_SIZE_M * SHUTTLE_SQUARE_SIZE_M;
float surveyTotalRouteM = 0.0f;
bool surveyRouteBuilt = false;

// One safe one-way photo strip at a time. The operator shifts the boat
// sideways at the shore, points it into the water, locks the bow, and runs
// only the selected straight strip. There are no automatic turns or lane changes.
uint8_t selectedSurveyLane = 0;
uint32_t selectedStripPhotosSavedAtStart = 0;
uint32_t outboundPhotosSavedAtTurn = 0;
uint32_t completedLaneMask = 0;
constexpr float SELECTED_STRIP_START_NEAR_M = 3.5f;

int16_t activeLegIndex = -1;
bool activeLegIsSurvey = true;
bool activeLegOutbound = true;
GeoPoint activeLegStartPoint;
GeoPoint activeLegEndPoint;
...
float activeLegMinimumTargetDistanceM = 9999.0f;
uint32_t activeLegHardStopMs = 0;
...
uint32_t surveyPhotoIntervalMs = 850;
...
constexpr uint32_t AUTO_MAX_RUN_TIME_MS = 8UL * 60UL * 1000UL;
constexpr float AUTO_MAX_DISTANCE_FROM_A_M = 35.0f;
...
constexpr float SURVEY_MIN_SPACING_M = 0.55f;
constexpr float SURVEY_MAX_SPACING_M = 1.20f;
constexpr uint32_t SURVEY_MIN_PHOTO_INTERVAL_MS = 700;
constexpr uint32_t SURVEY_MAX_PHOTO_INTERVAL_MS = 1800;
constexpr float SURVEY_ENDPOINT_RADIUS_M = 1.8f;
constexpr float SURVEY_LOOKAHEAD_M = 2.2f;
constexpr float SURVEY_HEADING_DEADBAND_DEG = 4.0f;
constexpr float SURVEY_HEADING_KP_US_PER_DEG = 0.55f;
constexpr float SURVEY_YAW_DAMPING_US_PER_DPS = 0.85f;
...
constexpr int SURVEY_BASE_US = 1410;
constexpr uint32_t STRAIGHT_LEG_MIN_TIME_MS = 1400;
constexpr uint32_t END_HOLD_MS = 900;
constexpr float ENDPOINT_CLOSEST_APPROACH_ARM_M = 3.8f;
constexpr float ENDPOINT_DISTANCE_RISE_M = 0.9f;

// Deliberately simple 180-degree pivot at the far end. Returning along the
// same GPS line removes the unreliable short connector used by lawnmower mode.
constexpr int TURN_FAST_FORWARD_US = 1370;
constexpr int TURN_FAST_REVERSE_US = 1630;
constexpr int TURN_SLOW_FORWARD_US = 1420;
constexpr int TURN_SLOW_REVERSE_US = 1580;
constexpr float TURN_ACCEPT_DEG = 11.0f;
constexpr float TURN_SLOW_ZONE_DEG = 35.0f;
constexpr float TURN_MAX_YAW_FOR_HOLD_DPS = 10.0f;
constexpr uint32_t TURN_ALIGNED_HOLD_MS = 650;
constexpr uint32_t TURN_TIMEOUT_MS = 22000;
...
// GPS course only corrects slow gyro drift while the boat is moving straight.
constexpr float HEADING_GPS_MIN_SPEED_KMPH = 0.90f;
constexpr float HEADING_GPS_MAX_YAW_RATE_DPS = 6.0f;
constexpr float HEADING_GPS_MAX_COURSE_DELTA_DEG = 30.0f;
constexpr float HEADING_GPS_CORRECTION_GAIN = 0.045f;
constexpr float HEADING_GPS_MAX_STEP_DEG = 0.9f;
...
String surveyStateName() {
  switch (surveyState) {
    ...
    case SurveyState::OUTBOUND: return "OUTBOUND";
    case SurveyState::FAR_HOLD: return "FAR_HOLD";
    case SurveyState::TURN_180: return "TURN_180";
    case SurveyState::RETURNING: return "RETURNING";
    case SurveyState::SHORE_HOLD: return "SHORE_HOLD";
...
bool surveyRunning() {
  return surveyState == SurveyState::OUTBOUND ||
         surveyState == SurveyState::FAR_HOLD;
}
...
void saveSurveyConfigToNvs() {
  ...
  preferences.putInt("waterSide", waterSideSign);
}
...
void loadSurveyConfigFromNvs() {
  ...
  surveyRequestedSpacingM = constrain(preferences.getFloat("surveySpace", 0.70f), SURVEY_MIN_SPACING_M, SURVEY_MAX_SPACING_M);
  surveyPhotoIntervalMs = constrain(preferences.getUInt("surveyPhoto", 850), SURVEY_MIN_PHOTO_INTERVAL_MS, SURVEY_MAX_PHOTO_INTERVAL_MS);
  waterSideSign = preferences.getInt("waterSide", 1) >= 0 ? 1 : -1;
}
...
void clearSurveyArea() {
  ...
  surveyLaneCount = 0;
  selectedSurveyLane = 0;
  completedLaneMask = 0;
  manualPhotoSweepEnabled = false;
  surveyAreaM2 = 100.0f;
  ...
  lastSystemEvent = "SHORE_POINTS_CLEARED";
}
...
bool buildSurveyRoute() {
  if (!surveyASet || !surveyDSet) {
    lastSystemEvent = "BUILD_BLOCKED_SAVE_SHORE_A_AND_D";
    return false;
  }

  LocalPoint rawD = geoToLocal(surveyA, surveyD);
  float rawShoreLength = localLength(rawD);
  if (rawShoreLength < 4.0f) {
    lastSystemEvent = "BUILD_SHORE_POINTS_TOO_CLOSE";
    return false;
  }

  LocalPoint shoreUnit = {rawD.x / rawShoreLength, rawD.y / rawShoreLength};
  LocalPoint leftNormal = {-shoreUnit.y, shoreUnit.x};
  LocalPoint waterUnit = waterSideSign > 0
    ? leftNormal
    : LocalPoint{-leftNormal.x, -leftNormal.y};

  // Use the measured shore line only for orientation and midpoint. The actual
  // square is forced to exactly 10 x 10 m, removing GPS length distortion.
  LocalPoint rawMid = {rawD.x * 0.5f, rawD.y * 0.5f};
  LocalPoint fitALocal = {
    rawMid.x - shoreUnit.x * (SHUTTLE_SQUARE_SIZE_M * 0.5f),
    rawMid.y - shoreUnit.y * (SHUTTLE_SQUARE_SIZE_M * 0.5f)
  };
  LocalPoint fitDLocal = {
    rawMid.x + shoreUnit.x * (SHUTTLE_SQUARE_SIZE_M * 0.5f),
    rawMid.y + shoreUnit.y * (SHUTTLE_SQUARE_SIZE_M * 0.5f)
  };
  LocalPoint fitBLocal = {
    fitALocal.x + waterUnit.x * SHUTTLE_SQUARE_SIZE_M,
    fitALocal.y + waterUnit.y * SHUTTLE_SQUARE_SIZE_M
  };
  LocalPoint fitCLocal = {
    fitDLocal.x + waterUnit.x * SHUTTLE_SQUARE_SIZE_M,
    fitDLocal.y + waterUnit.y * SHUTTLE_SQUARE_SIZE_M
  };

  fittedA = localToGeo(surveyA, fitALocal);
  fittedB = localToGeo(surveyA, fitBLocal);
  fittedC = localToGeo(surveyA, fitCLocal);
  fittedD = localToGeo(surveyA, fitDLocal);
  surveyB = fittedB;
  surveyC = fittedC;
  surveyBSet = true;
  surveyCSet = true;

  float usableWidth = SHUTTLE_SQUARE_SIZE_M - 2.0f * SHUTTLE_SIDE_MARGIN_M;
  uint8_t laneCount = static_cast<uint8_t>(ceilf(usableWidth / surveyRequestedSpacingM)) + 1U;
  laneCount = constrain(laneCount, static_cast<uint8_t>(2), MAX_SURVEY_LANES);
  float actualSpacing = usableWidth / static_cast<float>(laneCount - 1U);
  float activeLength = SHUTTLE_SQUARE_SIZE_M - SHUTTLE_SHORE_MARGIN_M - SHUTTLE_FAR_MARGIN_M;

  uint8_t pointIndex = 0;
  for (uint8_t lane = 0; lane < laneCount; lane++) {
    float across = SHUTTLE_SIDE_MARGIN_M + actualSpacing * static_cast<float>(lane);
    LocalPoint shorePoint = {
      fitALocal.x + shoreUnit.x * across,
      fitALocal.y + shoreUnit.y * across
    };
    LocalPoint start = {
      shorePoint.x + waterUnit.x * SHUTTLE_SHORE_MARGIN_M,
      shorePoint.y + waterUnit.y * SHUTTLE_SHORE_MARGIN_M
    };
    LocalPoint finish = {
      shorePoint.x + waterUnit.x * (SHUTTLE_SQUARE_SIZE_M - SHUTTLE_FAR_MARGIN_M),
      shorePoint.y + waterUnit.y * (SHUTTLE_SQUARE_SIZE_M - SHUTTLE_FAR_MARGIN_M)
    };
  ...
  surveyLengthM = SHUTTLE_SQUARE_SIZE_M;
  surveyWidthM = SHUTTLE_SQUARE_SIZE_M;
  surveyAreaM2 = 100.0f;
  surveyActualSpacingM = actualSpacing;
  surveyTotalRouteM = activeLength * static_cast<float>(laneCount);
  surveyRouteBuilt = true;
  selectedSurveyLane = min(selectedSurveyLane, static_cast<uint8_t>(laneCount - 1U));
  completedLaneMask = 0;
  headingInitialized = false;
  bowLockPending = false;
  surveyState = SurveyState::IDLE;
  saveSurveyConfigToNvs();
  lastSystemEvent = "10X10_MANUAL_ORTHO_GRID_BUILT_" + String(laneCount) + "_LANES";
  return true;
}
...
void configureSurvey(float spacingM, uint32_t photoIntervalMs) {
  ...
  bowLockPending = false;
  saveSurveyConfigToNvs();
  lastSystemEvent = "SHUTTLE_CONFIG_SAVED_REBUILD_GRID";
}



uint8_t selectedLaneLegIndex() {
  if (!surveyRouteBuilt || surveyLaneCount == 0) return 0;
  if (selectedSurveyLane >= surveyLaneCount) selectedSurveyLane = surveyLaneCount - 1;
  return static_cast<uint8_t>(selectedSurveyLane * 2U);
}

GeoPoint selectedLaneStart() {
  if (!surveyRouteBuilt || surveyRoutePointCount < 2) return GeoPoint{0.0, 0.0};
  return surveyRoute[selectedLaneLegIndex()];
}

GeoPoint selectedLaneEnd() {
  if (!surveyRouteBuilt || surveyRoutePointCount < 2) return GeoPoint{0.0, 0.0};
  return surveyRoute[selectedLaneLegIndex() + 1U];
}

void selectSurveyLane(int laneIndex) {
  stopAutonomy("LANE_SELECTION", false);
  manualPhotoSweepEnabled = false;
  if (!surveyRouteBuilt || surveyLaneCount == 0) {
    selectedSurveyLane = 0;
    lastSystemEvent = "SELECT_LANE_BLOCKED_BUILD_GRID";
    return;
  }
  laneIndex = constrain(laneIndex, 0, static_cast<int>(surveyLaneCount) - 1);
  selectedSurveyLane = static_cast<uint8_t>(laneIndex);
  activeLegIndex = selectedLaneLegIndex();
  activeLegIsSurvey = true;
  activeLegOutbound = true;
  setActiveTarget(selectedLaneEnd());
  headingInitialized = false;
  bowLockPending = false;
  surveyState = SurveyState::IDLE;
  autoMessage = "MOVE_TO_SHORE_START_POINT_OUTWARD";
  lastSystemEvent = "SELECTED_LANE_" + String(selectedSurveyLane + 1) + "_OF_" + String(surveyLaneCount);
}


void selectNextSurveyLane() {
  if (!surveyRouteBuilt || surveyLaneCount == 0) { selectSurveyLane(0); return; }
  int nextLane = min(static_cast<int>(surveyLaneCount) - 1, static_cast<int>(selectedSurveyLane) + 1);
  selectSurveyLane(nextLane);
}

void selectPreviousSurveyLane() {
  selectSurveyLane(max(0, static_cast<int>(selectedSurveyLane) - 1));
}

void setManualPhotoSweep(bool enabled) {
  if (enabled) {
    if (surveyRunning()) stopAutonomy("MANUAL_PHOTO_SWEEP_SELECTED", false);
    if (!cameraOnline || !cameraRemoteReady || !cameraSdReady) {
      manualPhotoSweepEnabled = false;
      lastSystemEvent = "PHOTO_SWEEP_BLOCKED_CAMERA_OR_SD";
      return;
    }
    if (!missionActive) {
      manualPhotoSweepEnabled = false;
      lastSystemEvent = "PHOTO_SWEEP_BLOCKED_START_MISSION";
      return;
    }
    manualPhotoSweepEnabled = true;
    manualPhotoLastRequestMs = 0;
    manualPhotoDistanceSinceLastM = 0.0f;
    manualPhotoSchedulerLastMs = millis();
    lastSystemEvent = "MANUAL_DENSE_PHOTOS_ON_FORWARD_ONLY";
  } else {
    manualPhotoSweepEnabled = false;
    lastSystemEvent = "MANUAL_PHOTO_SWEEP_OFF";
  }
}

void updateManualPhotoSweep() {
  if (!manualPhotoSweepEnabled || surveyRunning()) return;
  if (!motorsArmed || !manualDriveActive || manualDriveCommand != 'f') {
    manualPhotoSchedulerLastMs = millis();
    return;
  }
  if (!missionActive || !cameraOnline || !cameraRemoteReady || !cameraSdReady) return;

  uint32_t nowMs = millis();
  if (manualPhotoSchedulerLastMs == 0) manualPhotoSchedulerLastMs = nowMs;
  float dt = constrain(static_cast<float>(nowMs - manualPhotoSchedulerLastMs) / 1000.0f, 0.0f, 0.25f);
  manualPhotoSchedulerLastMs = nowMs;

  float speedMps = 0.22f;
  if (gps.speed.isValid()) {
    float measured = static_cast<float>(gps.speed.mps());
    if (measured >= 0.05f && measured <= 0.80f) speedMps = measured;
  }
  manualPhotoDistanceSinceLastM += speedMps * dt;

  if (captureRequestPending) return;
  bool firstPhoto = manualPhotoLastRequestMs == 0;
  uint32_t elapsed = firstPhoto ? ORTHO_HARD_PHOTO_DEADLINE_MS : nowMs - manualPhotoLastRequestMs;
  bool distanceDue = manualPhotoDistanceSinceLastM >= ORTHO_TARGET_PHOTO_STEP_M;
  bool timeDue = elapsed >= max(surveyPhotoIntervalMs, ORTHO_SOFT_PHOTO_DEADLINE_MS);
  bool hardDue = elapsed >= ORTHO_HARD_PHOTO_DEADLINE_MS;
  bool quality = surveyPhotoQualityOkay();

  if (firstPhoto || ((distanceDue || timeDue) && quality) || hardDue) {
    if (requestSurveyCapture()) {
      manualPhotoLastRequestMs = nowMs;
      manualPhotoDistanceSinceLastM = 0.0f;
    }
  }
}
...
bool chooseSurveyLockTarget(GeoPoint& target, bool& approachStart) {
  approachStart = false;
  if (!surveyRouteBuilt || surveyLaneCount == 0 || surveyRoutePointCount < 2 || !gps.location.isValid()) return false;
  GeoPoint start = selectedLaneStart();
  float startDistance = distanceToGeo(start);
  if (startDistance < 0.0f || startDistance > SELECTED_STRIP_START_NEAR_M) return false;
  target = selectedLaneEnd();
  return true;
}
...
bool startBowLockCalibration() {
  ...
  if (!chooseSurveyLockTarget(lockTarget, approachStart)) {
    lastSystemEvent = "BOW_LOCK_BLOCKED_BUILD_GRID_OR_MOVE_TO_SHORE_START";
    return false;
  }
  ...
  lastSystemEvent = approachStart ? "LOCK_BOW_TO_A_HOLD_2S" : "LOCK_BOW_OUTWARD_HOLD_2S";
  return true;
}
...
void updateBowLockCalibration() {
  ...
  currentHeadingErrorDeg = 0.0;
  bowLockPending = false;
  lastSystemEvent = "BOW_LOCKED_OUTWARD_SHUTTLE_READY";
}

void updateHeadingGpsCorrection() {
  if (!headingInitialized || surveyState != SurveyState::OUTBOUND) return;
  uint32_t nowMs = millis();
  ...
  if (abs(currentLeftUs - rightPreTrim) > 20) return;
  ...
  float gain = HEADING_GPS_CORRECTION_GAIN;
  float maxStep = HEADING_GPS_MAX_STEP_DEG;
...
bool surveyPhotoQualityOkay() {
  return levelEnough &&
         fabsf(relativeRollDegrees) <= 11.0f &&
         fabsf(relativePitchDegrees) <= 11.0f &&
         fabsf(currentGyroX) <= 6.0f &&
         fabsf(currentGyroY) <= 6.0f &&
         fabsf(currentGyroZ) <= 6.0f;
}

void updateSurveyCapture() {
  if (surveyState != SurveyState::OUTBOUND) return;
  ...
  uint32_t elapsed = firstPhoto ? ORTHO_HARD_PHOTO_DEADLINE_MS : nowMs - lastSurveyPhotoRequestMs;
  float distanceSincePhoto = firstPhoto ? ORTHO_TARGET_PHOTO_STEP_M
                                        : max(0.0f, activeLegAlongM - photoAlongAtLastRequestM);
  bool distanceDue = distanceSincePhoto >= ORTHO_TARGET_PHOTO_STEP_M;
  bool timeDue = elapsed >= max(surveyPhotoIntervalMs, ORTHO_SOFT_PHOTO_DEADLINE_MS);
  bool hardDue = elapsed >= ORTHO_HARD_PHOTO_DEADLINE_MS;
  bool quality = surveyPhotoQualityOkay();

  if (firstPhoto || ((distanceDue || timeDue) && quality) || hardDue) {
    if (requestSurveyCapture()) photoAlongAtLastRequestM = activeLegAlongM;
  }
}


void beginStraightLeg(const GeoPoint& start, const GeoPoint& finish, bool outbound) {
  activeLegStartPoint = start;
  activeLegEndPoint = finish;
  activeLegOutbound = outbound;
  activeLegIsSurvey = true;
  setActiveTarget(finish);
  activeLegLengthM = max(0.1f, localLength(geoToLocal(start, finish)));
  activeLegAlongM = 0.0f;
  activeLegCrossTrackM = 0.0f;
  activeLegProgressPercent = 0.0f;
  activeLegStartedMs = millis();
  surveyLegStartedMs = activeLegStartedMs;
  finalPhotoRequestedForLeg = false;
  activeLegMinimumTargetDistanceM = 9999.0f;
  steeringCorrectionUs = 0.0f;
  float conservativeSpeed = max(MIN_VALID_SPEED_MPS, estimatedSurveySpeedMps * 0.52f);
  uint32_t expectedMs = static_cast<uint32_t>((activeLegLengthM / conservativeSpeed) * 1000.0f);
  activeLegHardStopMs = activeLegStartedMs + constrain(expectedMs + 12000UL, 18000UL, 120000UL);
  lastSurveyPhotoRequestMs = 0;
  photoAlongAtLastRequestM = -1000.0f;
  laneExpectedPhotoCount = static_cast<uint32_t>(ceilf(activeLegLengthM / ORTHO_TARGET_PHOTO_STEP_M)) + 2U;
  laneSavedPhotoCount = 0;
  lanePhotoCoverageGood = false;
  surveyState = SurveyState::OUTBOUND;
  autoMessage = "GUIDED_ONE_WAY_PHOTO_STRIP";
}


void beginFarTurn() {
  stopMotors();
  float outboundBearing = static_cast<float>(TinyGPSPlus::courseTo(
    selectedLaneStart().lat, selectedLaneStart().lon,
    selectedLaneEnd().lat, selectedLaneEnd().lon
  ));
  turnTargetHeadingDeg = normalizeHeading360(outboundBearing + 180.0f);
  turnStartedMs = millis();
  turnAlignedStartedMs = 0;
  surveyState = SurveyState::TURN_180;
  autoMessage = "GYRO_180_TURN";
  lastSystemEvent = "TURN_180_STARTED";
}


String surveyStartBlockReason() {
  if (surveyRunning()) return "SHUTTLE_ALREADY_RUNNING";
  if (!surveyRouteBuilt || surveyLaneCount == 0 || surveyRoutePointCount < 2) return "BUILD_10X10_GRID";
  if (!motorsArmed) return "ARM_MOTORS";
  if (!gpsGoodForAuto()) return "GPS_QUALITY";
  if (!gyroSignVerified) return "VERIFY_GYRO_SIGN";
  if (!headingInitialized || bowLockPending) return "POINT_OUTWARD_AND_LOCK_BOW";
  ...
  if (!missionActive) return "START_NEW_PHOTO_MISSION";
  if (captureRequestPending) return "CAMERA_BUSY";
  float startDistance = distanceToGeo(selectedLaneStart());
  if (startDistance < 0.0f) return "GPS_POSITION";
  if (startDistance > SELECTED_STRIP_START_NEAR_M) return "MOVE_TO_SELECTED_SHORE_START";
  return "";
}
...
bool startSurvey() {
  ...
  lastCaptureResult = 0;
  consecutiveCaptureFailures = 0;
  selectedStripPhotosSavedAtStart = surveyPhotosSaved;
  outboundPhotosSavedAtTurn = surveyPhotosSaved;
  activeLegIndex = selectedLaneLegIndex();
  beginStraightLeg(selectedLaneStart(), selectedLaneEnd(), true);
  lastSystemEvent = "GUIDED_ORTHO_LANE_" + String(selectedSurveyLane + 1) + "_STARTED";
  return true;
}
...
void commandTurnToBearing(float desiredBearing) {
  ...
  if (absError <= TURN_ACCEPT_DEG && fabsf(yawRate) <= TURN_MAX_YAW_FOR_HOLD_DPS) {
    stopMotors();
  ...
  turnAlignedStartedMs = 0;
  bool slow = absError <= TURN_SLOW_ZONE_DEG;
  int forwardUs = slow ? TURN_SLOW_FORWARD_US : TURN_FAST_FORWARD_US;
  int reverseUs = slow ? TURN_SLOW_REVERSE_US : TURN_FAST_REVERSE_US;

  if (currentHeadingErrorDeg > 0.0) {
    // Right turn: left propeller forward, right propeller reverse.
    writeMotors(forwardUs, reverseUs);
  } else {
    // Left turn: left propeller reverse, right propeller forward.
    writeMotors(reverseUs, rightRawForDesiredFinal(forwardUs));
  }
}
...
float computeLegDesiredBearing() {
  ...
  LocalPoint end = geoToLocal(activeLegStartPoint, activeLegEndPoint);
  LocalPoint position = geoToLocal(activeLegStartPoint, current);
...
void updateActiveLegDistanceHistory() {
  if (currentTargetDistanceM >= 0.0 && currentTargetDistanceM < activeLegMinimumTargetDistanceM) {
    activeLegMinimumTargetDistanceM = static_cast<float>(currentTargetDistanceM);
  }
}

void updateSurveySpeedEstimate() {
  if (!activeLegIsSurvey || !gps.speed.isValid()) return;
  float speedMps = static_cast<float>(gps.speed.mps());
  if (speedMps < MIN_VALID_SPEED_MPS || speedMps > MAX_VALID_SPEED_MPS) return;
  estimatedSurveySpeedMps = 0.85f * estimatedSurveySpeedMps + 0.15f * speedMps;
}
...
void configureActiveLegStopTimes(uint32_t nowMs) {
  ...
  float conservativeSpeed = max(MIN_VALID_SPEED_MPS, estimatedSurveySpeedMps * 0.52f);
  uint32_t expectedMs = static_cast<uint32_t>((activeLegLengthM / conservativeSpeed) * 1000.0f);
  activeLegHardStopMs = nowMs + constrain(expectedMs + 12000UL, 18000UL, 120000UL);
}


bool activeLegReached() {
  uint32_t elapsedMs = millis() - activeLegStartedMs;
  updateActiveLegDistanceHistory();
  if (elapsedMs < STRAIGHT_LEG_MIN_TIME_MS) return false;

  if (currentTargetDistanceM >= 0.0 && currentTargetDistanceM <= SURVEY_ENDPOINT_RADIUS_M) {
    lastSystemEvent = activeLegOutbound ? "FAR_RADIUS_REACHED" : "SHORE_RADIUS_REACHED";
    return true;
  }
  if (activeLegAlongM >= activeLegLengthM - 0.55f) {
    lastSystemEvent = activeLegOutbound ? "FAR_END_PLANE_REACHED" : "SHORE_END_PLANE_REACHED";
    return true;
  }
  if (activeLegMinimumTargetDistanceM <= ENDPOINT_CLOSEST_APPROACH_ARM_M &&
      currentTargetDistanceM >= 0.0 &&
      currentTargetDistanceM >= activeLegMinimumTargetDistanceM + ENDPOINT_DISTANCE_RISE_M) {
    lastSystemEvent = activeLegOutbound ? "FAR_CLOSEST_APPROACH_PASSED" : "SHORE_CLOSEST_APPROACH_PASSED";
    return true;
  }
  return false;
}
...
void driveCurrentLeg() {
  ...
  float yawRate = static_cast<float>(gyroCompassSign) * currentGyroZ;
  float targetCorrection = SURVEY_HEADING_KP_US_PER_DEG * static_cast<float>(currentHeadingErrorDeg)
                         - SURVEY_YAW_DAMPING_US_PER_DPS * yawRate;
  ...
  int baseUs = SURVEY_BASE_US;
  float absError = fabsf(static_cast<float>(currentHeadingErrorDeg));
  if (millis() - activeLegStartedMs < 1400UL) baseUs = max(baseUs, 1390);
  if (absError > 25.0f) baseUs = max(baseUs, 1410);
  if (absError > 50.0f) baseUs = max(baseUs, 1435);
...
void advanceAfterWaypoint() {
  stopMotors();
  completedLaneMask |= (1UL << selectedSurveyLane);
  surveyState = SurveyState::COMPLETE;
  autoRunStartedMs = 0;
  headingInitialized = false;
  bowLockPending = false;

  laneSavedPhotoCount = surveyPhotosSaved - selectedStripPhotosSavedAtStart;
  uint32_t minimumGood = static_cast<uint32_t>(ceilf(static_cast<float>(laneExpectedPhotoCount) * 0.72f));
  lanePhotoCoverageGood = laneSavedPhotoCount >= minimumGood;
  autoMessage = lanePhotoCoverageGood
    ? "LANE_GOOD_MOVE_SIDEWAYS_NEXT_AND_RELOCK"
    : "LANE_LOW_PHOTO_COUNT_REPEAT_THIS_LANE";
  lastSystemEvent = "LANE_" + String(selectedSurveyLane + 1)
                  + (lanePhotoCoverageGood ? "_GOOD_" : "_REPEAT_")
                  + String(laneSavedPhotoCount) + "_OF_EXPECTED_" + String(laneExpectedPhotoCount);
}
...
void updateAutopilot() {
  ...
  if (autoRunStartedMs != 0 && nowMs - autoRunStartedMs > AUTO_MAX_RUN_TIME_MS) { stopAutonomy("LANE_TIME_LIMIT", true); return; }
  if (distanceToGeo(fittedA) > AUTO_MAX_DISTANCE_FROM_A_M) { stopAutonomy("GEOFENCE", true); return; }
  ...
  if (captureRequestPending && nowMs - captureRequestStartedMs > AUTO_CAPTURE_TIMEOUT_MS) {
    bool wasSurvey = pendingCaptureIsSurvey;
    captureRequestPending = false;
    pendingCaptureIsSurvey = false;
    if (wasSurvey) {
      surveyPhotosFailed++;
    ...
    lastSystemEvent = "CAPTURE_TIMEOUT_CONTINUE_" + String(consecutiveCaptureFailures);
    if (consecutiveCaptureFailures >= MAX_CONSECUTIVE_CAPTURE_FAILURES) {
      stopAutonomy("CAMERA_FAILED_5_TIMES", true);
      return;
    }
  }
  ...
  if (surveyState == SurveyState::OUTBOUND) {
    driveCurrentLeg();
    updateSurveySpeedEstimate();
    updateSurveyCapture();

    if (activeLegHardStopMs != 0 && static_cast<int32_t>(nowMs - activeLegHardStopMs) >= 0) {
      stopAutonomy("LANE_HARD_TIMEOUT", true);
      return;
    }
    ...
    if (activeLegReached()) {
      ...
      finalPhotoRequestedForLeg = false;
      surveyState = SurveyState::FAR_HOLD;
      autoMessage = "FAR_END_STOP_FINAL_PHOTO";
    }
    return;
  }

  if (surveyState == SurveyState::FAR_HOLD) {
    stopMotors();
    if (!finalPhotoRequestedForLeg && !captureRequestPending && nowMs - waypointHoldStartedMs >= 350UL) {
      finalPhotoRequestedForLeg = true;
      requestSurveyCapture();
    }
    if (nowMs - waypointHoldStartedMs >= END_HOLD_MS && !captureRequestPending) {
...
void printNavigationStatus() {
  Serial.println();
  Serial.println("========== 10x10 MANUAL ORTHOPHOTO ==========");
  Serial.print("State: "); Serial.println(surveyStateName());
  Serial.print("Grid built: "); Serial.println(surveyRouteBuilt ? "YES" : "NO");
  Serial.print("Water side from A->D: "); Serial.println(waterSideSign > 0 ? "LEFT" : "RIGHT");
  Serial.print("Selected lane: "); Serial.print(selectedSurveyLane + 1); Serial.print("/"); Serial.println(surveyLaneCount);
  Serial.print("Lane spacing: "); Serial.println(surveyActualSpacingM, 2);
  Serial.print("Move sideways after lane: "); Serial.print(surveyActualSpacingM, 2); Serial.println(" m toward D");
  Serial.print("Distance to shore start: "); Serial.println(surveyRouteBuilt ? distanceToGeo(selectedLaneStart()) : -1.0f, 1);
  Serial.println("Direction: SHORE_TO_FAR_EDGE_ONE_WAY");
  ...
  Serial.print("Lane expected/saved/quality: "); Serial.print(laneExpectedPhotoCount); Serial.print("/"); Serial.print(laneSavedPhotoCount); Serial.print("/"); Serial.println(lanePhotoCoverageGood ? "GOOD" : "CHECK_OR_REPEAT");
  Serial.println("=========================================");
}
...
void printHelp() {
  Serial.println();
  Serial.println("========== 10x10 MANUAL ORTHOPHOTO COMMANDS ==========");
  Serial.println("M / E                 - start/end camera mission");
  Serial.println("ARM / DISARM / STOP");
  Serial.println("A_HERE / D_HERE       - average the two shore corners");
  Serial.println("WATERLEFT / WATERRIGHT- water side while facing A -> D");
  Serial.println("BUILD                  - build exact 10 x 10 grid");
  Serial.println("PREV / NEXT            - choose lane");
  Serial.println("LOCKBOW                 - point away from shore and hold 2 s");
  Serial.println("SURVEY                 - one straight shore-to-far-edge photo lane");
  Serial.println("PHOTOON / PHOTOOFF      - dense manual photos on FORWARD only");
  Serial.println("NAV                     - print status");
  Serial.println("==================================================");
}
...
void processUsbCommand(String command) {
  ...
  else if (upper == "STOP" || upper == "ABORT") { manualPhotoSweepEnabled = false; stopAutonomy("USER_ABORT", false); }
  ...
  else if (upper == "WATERLEFT") { waterSideSign = 1; surveyRouteBuilt = false; saveSurveyConfigToNvs(); lastSystemEvent = "WATER_SIDE_LEFT"; }
  else if (upper == "WATERRIGHT") { waterSideSign = -1; surveyRouteBuilt = false; saveSurveyConfigToNvs(); lastSystemEvent = "WATER_SIDE_RIGHT"; }
  else if (upper == "BUILD") buildSurveyRoute();
  else if (upper == "PREV") selectPreviousSurveyLane();
  else if (upper == "NEXT") selectNextSurveyLane();
  else if (upper == "LOCKBOW") startBowLockCalibration();
  else if (upper == "SHUTTLE" || upper == "SURVEY") startSurvey();
  else if (upper == "PHOTOON") setManualPhotoSweep(true);
  else if (upper == "PHOTOOFF") setManualPhotoSweep(false);
...
<style>
html,body{overscroll-behavior:none;-webkit-touch-callout:none}body{font-family:Arial;background:#0f1419;color:#fff;text-align:center;margin:0;padding:9px;-webkit-user-select:none;user-select:none}.card{background:#1d2730;border-radius:14px;padding:12px;margin:9px auto;max-width:760px}button,input{font-size:17px;padding:13px;margin:5px;border:0;border-radius:11px}button{min-width:125px;touch-action:manipulation;-webkit-user-select:none;user-select:none;-webkit-tap-highlight-color:transparent}button.hold{touch-action:none}.go{background:#23a566;color:#fff}.stop{background:#d62828;color:#fff}.manual{background:#74c0fc}.mode{background:#f0b429}.safe{background:#6c757d;color:#fff}.photo{background:#8f5bd7;color:#fff}.water{background:#1f8ac0;color:#fff}input{width:138px;-webkit-user-select:text;user-select:text}.mono{font-family:monospace;text-align:left;white-space:pre-wrap;font-size:13px}.small{font-size:13px;color:#cbd5e1}.warn{color:#ffd166}.ready{font-weight:700;font-size:20px;padding:10px;border-radius:10px;background:#6c2}.blocked{font-weight:700;font-size:18px;padding:10px;border-radius:10px;background:#a33}
</style></head><body>
<h2>10×10 Manual Orthophoto V1.0</h2>
...
<div class="card"><h3>1. Camera quality first</h3><button class="mode" onclick="cmd('camerastatus')">REFRESH CAMERA / SD</button><br><button class="safe" onclick="cmd('missionend')">END OLD MISSION</button><button class="go" onclick="cmd('missionstart')">START NEW MISSION</button><button class="photo" onclick="cmd('capturetest')">TEST PHOTO</button><p class="warn">Open the TEST PHOTO at full size. Do not survey until stones or bottom texture have sharp edges while the boat is stationary at the real depth.</p></div>
<div class="card"><h3>2. Exact 10×10 square from shore</h3><p class="small">Save A at the left shore corner and D at the right shore corner. Stand at A looking toward D and choose the side containing water. The program forces an exact 10×10 m square and creates every lane itself.</p><button class="mode" onclick="cmd('savea')">SAVE SHORE A</button><button class="mode" onclick="cmd('saved')">SAVE SHORE D</button><br><button class="water" onclick="cmd('waterleft')">WATER IS LEFT</button><button class="water" onclick="cmd('waterright')">WATER IS RIGHT</button><br><button class="safe" onclick="cmd('cleararea')">CLEAR SHORE POINTS</button></div>
<div class="card"><h3>3. Dense grid</h3><label>Lane spacing, m<br><input id="spacing" type="number" step="0.05" min="0.55" max="1.20" value="0.70"></label><br><label>Nominal photo interval, ms<br><input id="photo" type="number" step="50" min="700" max="1800" value="850"></label><p class="small">Recommended: 0.70 m and 850 ms. The scheduler targets a frame every 0.20 m, waits briefly for low rotation, and forces a frame by 1.25 s to prevent holes.</p><button class="mode" onclick="saveConfig()">SAVE SETTINGS</button><button class="go" onclick="cmd('build')">BUILD EXACT 10×10 GRID</button></div>
<div class="card"><h3>4. Highest-quality guided strip</h3><button class="mode" onclick="cmd('prevlane')">PREVIOUS LANE</button><button class="mode" onclick="cmd('nextlane')">NEXT LANE</button><br><button class="mode" onclick="cmd('levelzero')">SET WATER LEVEL ZERO</button><button class="go" onclick="cmd('lockbow')">LOCK BOW OUTWARD</button><br><button class="go" onclick="cmd('survey')">START ONE-WAY PHOTO LANE</button><p class="small">The boat runs only the selected straight lane and stops at the far edge. No automatic turn. Bring it back manually with photos off, shift toward D, select NEXT LANE, relock, and repeat.</p></div>
<div class="card"><h3>5. Full manual fallback</h3><p class="small">Enable AUTO PHOTOS and hold SLOW FORWARD. Photos are made only during forward motion; LEFT, RIGHT and REVERSE never trigger a frame.</p><button class="photo" onclick="cmd('photoon')">AUTO PHOTOS ON</button><button class="safe" onclick="cmd('photooff')">AUTO PHOTOS OFF</button><br><button class="manual hold" data-m="f">SLOW FORWARD + PHOTOS</button><br><button class="manual hold" data-m="l">LEFT</button><button class="stop" onclick="manualStop()">STOP</button><button class="manual hold" data-m="r">RIGHT</button><br><button class="manual hold" data-m="b">REVERSE</button></div>
<div class="card"><h3>6. Motor balance</h3><button class="mode" onclick="cmd('trimminus')">RIGHT -5 us</button><button class="mode" onclick="cmd('trimreset')">RESET 20</button><button class="mode" onclick="cmd('trimplus')">RIGHT +5 us</button></div>
...
async function update(){try{const s=await(await fetch('/status',{cache:'no-store'})).json();document.getElementById('spacing').value=s.requestedSpacing;document.getElementById('photo').value=s.photoInterval;const box=document.getElementById('readyBox');box.textContent=s.ready==='YES'?'LANE READY':'NOT READY';box.className=s.ready==='YES'?'ready':'blocked';document.getElementById('reason').textContent=s.ready==='YES'?'Press START ONE-WAY PHOTO LANE':'Next action: '+s.blockReason;document.getElementById('status').textContent=
'GPS:'+s.gps+' SAT:'+s.sat+' HDOP:'+s.hdop+' SPEED:'+s.speed+' km/h\n'+
'CAM:'+s.camera+' READY:'+s.cameraReady+' SD:'+s.sd+' MISSION:'+s.mission+'\n'+
'SHORE A:'+s.aSet+' D:'+s.dSet+' WATER:'+s.waterSide+' GRID:'+s.routeBuilt+'\n'+
'GRID:10x10 m LANES:'+s.lanes+' SPACING:'+s.actualSpacing+' m ONE-WAY TOTAL:'+s.routeDistance+' m\n'+
'SELECTED:'+s.selectedLane+'/'+s.lanes+' START DIST:'+s.startDistance+' m SHIFT NEXT:'+s.nextShift+' m toward D\n'+
'AUTO:'+s.auto+' '+s.autoMessage+' PROGRESS:'+s.progress+'% XTE:'+s.xte+' m\n'+
'BOW:'+s.bowLock+' HEADING:'+s.heading+' DESIRED:'+s.desired+' ERROR:'+s.headingError+'\n'+
'MOTORS:'+s.motors+' L/R:'+s.left+'/'+s.right+' TRIM:'+s.rightTrim+' us\n'+
'LANE PHOTOS expected/saved/quality:'+s.laneExpected+'/'+s.laneSaved+'/'+s.laneQuality+'\n'+
'ALL PHOTOS requested/saved/failed:'+s.photosRequested+'/'+s.photosSaved+'/'+s.photosFailed+' FAIL STREAK:'+s.failStreak+' PENDING:'+s.capturePending+'\n'+
'EVENT:'+s.event+'\nPOINT AVERAGING:'+s.pointAveraging+' ('+s.avgSamples+' samples)'}catch(e){document.getElementById('status').textContent='Connection lost'}}setInterval(update,600);update();
</script></body></html>
)rawliteral";
}
...
void handleControlStatus() {
  ...
  String avgState = surveyPointAveragingSlot == SurveyPointSlot::NONE ? "NO" :
                    ...
                    surveyPointAveragingSlot == SurveyPointSlot::D ? "D" : "OTHER";
  String blockReason = surveyStartBlockReason();
  String readyState = blockReason.length() == 0 ? "YES" : "NO";
  float startDistance = surveyRouteBuilt && surveyLaneCount > 0 ? distanceToGeo(selectedLaneStart()) : -1.0f;
    String json = "{";
  json += "\"ready\":\"" + readyState + "\",\"blockReason\":\"" + blockReason + "\",";
  json += "\"gps\":\"" + gpsState + "\",\"sat\":" + String(gps.satellites.isValid() ? gps.satellites.value() : 0) + ",";
  json += "\"hdop\":" + jsonNumber(gps.hdop.isValid() ? gps.hdop.hdop() : 99.99, 2) + ",\"speed\":" + jsonNumber(gps.speed.isValid() ? gps.speed.kmph() : 0.0, 2) + ",";
  json += "\"camera\":\"" + String(cameraOnline ? "ONLINE" : "OFFLINE") + "\",\"cameraReady\":\"" + String(cameraRemoteReady ? "YES" : "NO") + "\",\"sd\":\"" + String(cameraSdReady ? "READY" : "NOT_READY") + "\",\"mission\":\"" + missionState + "\",";
  json += "\"motors\":\"" + String(motorsArmed ? "ARMED" : "SAFE") + "\",\"left\":" + String(currentLeftUs) + ",\"right\":" + String(currentRightUs) + ",\"rightTrim\":" + String(rightForwardTrimUs) + ",";
  json += "\"bowLock\":\"" + bowLockState + "\",\"heading\":" + jsonNumber(headingInitialized ? headingEstimateDeg : 0.0, 1) + ",";
  json += "\"aSet\":\"" + String(surveyASet ? "YES" : "NO") + "\",\"dSet\":\"" + String(surveyDSet ? "YES" : "NO") + "\",\"waterSide\":\"" + String(waterSideSign > 0 ? "LEFT" : "RIGHT") + "\",";
  json += "\"routeBuilt\":\"" + String(surveyRouteBuilt ? "YES" : "NO") + "\",\"lanes\":" + String(surveyLaneCount) + ",\"selectedLane\":" + String(surveyLaneCount > 0 ? selectedSurveyLane + 1 : 0) + ",";
  json += "\"requestedSpacing\":" + jsonNumber(surveyRequestedSpacingM, 2) + ",\"actualSpacing\":" + jsonNumber(surveyActualSpacingM, 2) + ",\"photoInterval\":" + String(surveyPhotoIntervalMs) + ",\"routeDistance\":" + jsonNumber(surveyTotalRouteM, 1) + ",";
  json += "\"startDistance\":" + jsonNumber(startDistance, 1) + ",\"nextShift\":" + jsonNumber(surveyActualSpacingM, 2) + ",";
  json += "\"auto\":\"" + surveyStateName() + "\",\"autoMessage\":\"" + autoMessage + "\",";
  json += "\"progress\":" + jsonNumber(activeLegProgressPercent, 1) + ",\"xte\":" + jsonNumber(activeLegCrossTrackM, 2) + ",\"desired\":" + jsonNumber(desiredTrackBearingDeg, 1) + ",\"headingError\":" + jsonNumber(currentHeadingErrorDeg, 1) + ",\"turnTarget\":" + jsonNumber(turnTargetHeadingDeg, 1) + ",";
  json += "\"laneExpected\":" + String(laneExpectedPhotoCount) + ",\"laneSaved\":" + String(laneSavedPhotoCount) + ",\"laneQuality\":\"" + String(lanePhotoCoverageGood ? "GOOD" : "CHECK") + "\",";
  json += "\"photosRequested\":" + String(surveyPhotosRequested) + ",\"photosSaved\":" + String(surveyPhotosSaved) + ",\"photosFailed\":" + String(surveyPhotosFailed) + ",\"failStreak\":" + String(consecutiveCaptureFailures) + ",\"capturePending\":" + String(captureRequestPending ? "true" : "false") + ",";
...
void handleControlCommand() {
  ...
  else if (command == "disarm") { manualPhotoSweepEnabled = false; disarmMotors(); }
  else if (command == "stop") { motorTestStopAtMs = 0; manualPhotoSweepEnabled = false; stopAutonomy("WEB_EMERGENCY_STOP", false); }
  ...
  else if (command == "waterleft") { waterSideSign = 1; surveyRouteBuilt = false; saveSurveyConfigToNvs(); lastSystemEvent = "WATER_SIDE_LEFT"; }
  else if (command == "waterright") { waterSideSign = -1; surveyRouteBuilt = false; saveSurveyConfigToNvs(); lastSystemEvent = "WATER_SIDE_RIGHT"; }
  else if (command == "cleararea") clearSurveyArea();
  else if (command == "build") buildSurveyRoute();
  else if (command == "prevlane") selectPreviousSurveyLane();
  else if (command == "nextlane") selectNextSurveyLane();
  ...
  else if (command == "missionend") { manualPhotoSweepEnabled = false; requestMissionEnd(); }
  else if (command == "photoon") setManualPhotoSweep(true);
  else if (command == "photooff") setManualPhotoSweep(false);
...
void printSystemState() {
  ...
  Serial.print(" STRIP:"); Serial.print(surveyLaneCount > 0 ? selectedSurveyLane + 1 : 0); Serial.print("/"); Serial.print(surveyLaneCount);
  Serial.print(" | MANPHOTO:"); Serial.print(manualPhotoSweepEnabled ? "ON" : "OFF");
...
void setup() {
  ...
  Serial.println("10x10 MANUAL ORTHOPHOTO V1.0: DENSE ONE-WAY LANES");
  Serial.println("======================================");
