...
constexpr uint32_t AUTO_COURSE_ACQUIRE_TIMEOUT_MS = 6000;
constexpr uint32_t AUTO_SETTLE_MIN_MS = 900;
constexpr uint32_t AUTO_CAPTURE_TIMEOUT_MS = 15000;
constexpr float AUTO_ARRIVAL_RADIUS_M = 4.0f;
constexpr float AUTO_MAX_ROLL_DEG = 22.0f;
constexpr float AUTO_MAX_PITCH_DEG = 22.0f;
constexpr float AUTO_HEADING_KP_US_PER_DEG = 1.8f;
constexpr float AUTO_MAX_CORRECTION_US = 125.0f;
...
void driveTowardTarget() {
  if (!gpsCourseReady()) {
    // Maintain slow straight motion until GPS has a usable course.
    writeMotors(AUTO_SLOW_US, AUTO_SLOW_US);
    return;
  }

  const float currentCourse = gps.course.deg();
  ...
  if (currentTargetDistanceM > 12.0) {
    baseUs = AUTO_FAST_US;
  } else if (currentTargetDistanceM < 7.0) {
    baseUs = AUTO_SLOW_US;
  }

  // Large heading errors: reduce forward speed so differential thrust can turn.
  if (fabsf(currentHeadingErrorDeg) > 65.0f) {
    baseUs = 1410;
  }

  float correction =
    AUTO_HEADING_KP_US_PER_DEG * currentHeadingErrorDeg;
  correction = constrain(
    correction,
    -AUTO_MAX_CORRECTION_US,
    AUTO_MAX_CORRECTION_US
  );

  // Positive error = target is to the right:
  // left motor gets more forward thrust (smaller pulse), right gets less.
  int leftUs = static_cast<int>(baseUs - correction);
  int rightUs = static_cast<int>(baseUs + correction);
...
async function update(){
 try{
  ...
  document.getElementById('status').textContent=
   ...
   'LAT: '+s.lat+'\nLON: '+s.lon+'\n'+
   'CAMERA: '+s.camera+'  MISSION: '+s.mission+'\n'+
   'MOTORS: '+s.motors+'  L/R: '+s.left+'/'+s.right+'\n'+
   'AUTO: '+s.auto+'  DIST: '+s.distance+' m\n'+
...
void setup() {
  ...
  Serial.println("AUTONOMY V4.1: WIFI WATER TEST + GPS + MPU + CAMERA + ESC");
  Serial.println("======================================");
