...
// Reliable raw-GPS controller. V4.2 filtering/derivative added too much delay.
constexpr float AUTO_HEADING_KP_US_PER_DEG = 0.90f;
constexpr float AUTO_HEADING_KD_US_PER_DEG_PER_SEC = 0.0f;
constexpr float AUTO_MAX_CORRECTION_US = 90.0f;
constexpr float AUTO_STEERING_SLEW_US_PER_UPDATE = 10.0f;
...
void driveTowardTarget() {
  ...
  // Limit steering according to the size of the error. This avoids the
  // delayed GPS course causing repeated full left/right commands.
  float dynamicMaxCorrection = 35.0f;
  if (absoluteError >= 20.0f) dynamicMaxCorrection = 55.0f;
  if (absoluteError >= 45.0f) dynamicMaxCorrection = 75.0f;
  if (absoluteError >= 80.0f) dynamicMaxCorrection = AUTO_MAX_CORRECTION_US;
  ...
  // Smooth only the motor command. Do not smooth the GPS course itself:
  // NEO-6M already reports it with delay.
  ...
  // Slow down while making a large correction. The boat keeps moving, so
  // GPS course remains valid, but it cannot build up a wide snake turn.
  if (absoluteError > 25.0f) baseUs = max(baseUs, 1380);
  if (absoluteError > 50.0f) baseUs = max(baseUs, 1410);
  if (absoluteError > 85.0f) baseUs = max(baseUs, 1430);

  // Positive error means the target is to the right of the movement course:
...
void setup() {
  ...
  Serial.println("AUTONOMY V4.3: RELIABLE RAW GPS + WATER PHOTO + TOUCH CONTROL");
  Serial.println("======================================");
