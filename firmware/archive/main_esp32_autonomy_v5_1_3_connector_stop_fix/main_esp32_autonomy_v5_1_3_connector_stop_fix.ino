...
// ============================================================
// V5.1.3 match survey controller (connector stop + runaway protection)
...
// Endpoint protection. Short lane-to-lane connectors are too small for a
// standard GPS receiver to stop on reliably, so they use a measured-speed
// time estimate with GPS as an early-stop aid. Every leg also has a hard
// deadline so the boat cannot continue indefinitely if GPS geometry stalls.
float activeLegMinimumTargetDistanceM = 9999.0f;
uint32_t activeLegPlannedStopMs = 0;
...
// Connector and endpoint safety. GPS error is often similar to the 1-2 m
// connector length, therefore connector completion must not depend on a
// small GPS radius alone.
...
constexpr float CONNECTOR_SPEED_FACTOR = 0.72f;
constexpr uint32_t CONNECTOR_MIN_DRIVE_MS = 2200;
constexpr uint32_t CONNECTOR_MAX_DRIVE_MS = 30000;
constexpr uint32_t CONNECTOR_EXTRA_HARD_STOP_MS = 5000;
constexpr uint32_t SURVEY_MIN_TRACK_MS = 1400;
constexpr float ENDPOINT_CLOSEST_APPROACH_ARM_M = 4.5f;
constexpr float ENDPOINT_DISTANCE_RISE_M = 1.0f;
...
void beginRouteLeg(uint8_t legIndex) {
  ...
  activeLegPlannedStopMs = 0;
  activeLegHardStopMs = 0;
  steeringCorrectionUs = 0.0f;
...
void configureActiveLegStopTimes(uint32_t nowMs) {
  ...
  if (activeLegIsSurvey) {
    float conservativeSpeed = max(MIN_VALID_SPEED_MPS, estimatedSurveySpeedMps * 0.55f);
    uint32_t expectedMs = static_cast<uint32_t>((activeLegLengthM / conservativeSpeed) * 1000.0f);
    activeLegHardStopMs = nowMs + constrain(expectedMs + 12000UL, 15000UL, 8UL * 60UL * 1000UL);
    return;
  }

  float connectorSpeed = max(0.16f, estimatedSurveySpeedMps * CONNECTOR_SPEED_FACTOR);
  uint32_t connectorMs = static_cast<uint32_t>((activeLegLengthM / connectorSpeed) * 1000.0f);
  connectorMs = constrain(connectorMs, CONNECTOR_MIN_DRIVE_MS, CONNECTOR_MAX_DRIVE_MS);
  activeLegPlannedStopMs = nowMs + connectorMs;
  activeLegHardStopMs = activeLegPlannedStopMs + CONNECTOR_EXTRA_HARD_STOP_MS;
}

bool activeLegReached() {
  if (activeLegIndex < 0) {
    return currentTargetDistanceM >= 0.0 && currentTargetDistanceM <= SURVEY_START_REACHED_M;
  }

  uint32_t nowMs = millis();
  uint32_t elapsedMs = nowMs - activeLegStartedMs;
  updateActiveLegDistanceHistory();

  if (!activeLegIsSurvey) {
    // Do not trust a 1-2 m GPS radius as the only connector stop condition.
    // The timed stop is derived from the speed measured on the previous strip.
    if (activeLegPlannedStopMs != 0 && static_cast<int32_t>(nowMs - activeLegPlannedStopMs) >= 0) {
      lastSystemEvent = "CONNECTOR_TIME_COMPLETE";
      return true;
    }

    uint32_t minimumGpsAssistMs = activeLegPlannedStopMs > activeLegStartedMs
      ? (activeLegPlannedStopMs - activeLegStartedMs) / 3UL
      : CONNECTOR_MIN_DRIVE_MS / 3UL;

    if (elapsedMs >= minimumGpsAssistMs) {
      if (activeLegAlongM >= activeLegLengthM * 0.72f) {
        lastSystemEvent = "CONNECTOR_GPS_PROGRESS_COMPLETE";
        return true;
      }
      if (activeLegMinimumTargetDistanceM <= SURVEY_CONNECTOR_RADIUS_M &&
          currentTargetDistanceM >= 0.0 &&
          currentTargetDistanceM >= activeLegMinimumTargetDistanceM + 0.7f) {
        lastSystemEvent = "CONNECTOR_CLOSEST_APPROACH_COMPLETE";
        return true;
      }
    }
    return false;
  ...
  if (elapsedMs < SURVEY_MIN_TRACK_MS) return false;

  if (currentTargetDistanceM >= 0.0 && currentTargetDistanceM <= SURVEY_ENDPOINT_RADIUS_M) {
    lastSystemEvent = "STRIP_ENDPOINT_RADIUS_REACHED";
    return true;
  }

  // Crossing the end plane is more reliable than waiting for a tiny circular
  // GPS radius when the boat is laterally offset from the strip.
  if (activeLegAlongM >= activeLegLengthM - 0.8f) {
    lastSystemEvent = "STRIP_END_PLANE_REACHED";
    return true;
  }

  // If the boat came close to the endpoint and the distance then increases,
  // it has passed the point and must stop instead of continuing away.
  if (activeLegMinimumTargetDistanceM <= ENDPOINT_CLOSEST_APPROACH_ARM_M &&
    ...
    lastSystemEvent = "STRIP_CLOSEST_APPROACH_PASSED";
    return true;
  }
  ...
  switch (surveyState) {
    ...
    case SurveyState::TURN_TO_LEG: {
      ...
      if (turnAlignmentComplete()) {
        ...
        lastSurveyPhotoRequestMs = 0;
        steeringCorrectionUs = 0.0f;
        configureActiveLegStopTimes(nowMs);
        autoMessage = activeLegIsSurvey ? "CAPTURING_STRIP" : "MOVE_TO_NEXT_STRIP_TIMED";
      } else if (nowMs - activeLegStartedMs > TURN_TIMEOUT_MS) stopAutonomy("TURN_LEG_TIMEOUT", true);
      break;
    }
    case SurveyState::TRACK_LEG:
      // Update geometry first, then decide whether the endpoint has been
      // reached. This avoids evaluating stale along-track data.
      driveCurrentLeg();
      updateSurveySpeedEstimate();
      updateSurveyCapture();

      if (activeLegHardStopMs != 0 && static_cast<int32_t>(nowMs - activeLegHardStopMs) >= 0) {
        stopAutonomy(activeLegIsSurvey ? "STRIP_HARD_TIMEOUT" : "CONNECTOR_HARD_TIMEOUT", true);
        return;
      }
      ...
      if (activeLegReached()) {
        ...
        finalPhotoRequestedForLeg = false;
        surveyState = SurveyState::WAYPOINT_HOLD;
        autoMessage = activeLegIsSurvey ? "STRIP_ENDPOINT_REACHED" : "CONNECTOR_COMPLETE";
      }
      break;
...
<h2>Catamaran V5.1.3 Match Survey</h2>
...
'LEG TIME:'+s.legElapsed+' ms STOP IN:'+s.stopIn+' ms EST SPEED:'+s.estimatedSpeed+' m/s\n'+
...
void handleControlStatus() {
  ...
  uint32_t nowMs = millis();
  uint32_t legElapsedMs = surveyRunning() ? nowMs - activeLegStartedMs : 0;
  int32_t stopInMs = 0;
  if (surveyRunning() && activeLegPlannedStopMs != 0) {
    stopInMs = static_cast<int32_t>(activeLegPlannedStopMs - nowMs);
    if (stopInMs < 0) stopInMs = 0;
  }
  String json = "{";
  ...
  json += "\"legElapsed\":" + String(legElapsedMs) + ",\"stopIn\":" + String(stopInMs) + ",\"estimatedSpeed\":" + jsonNumber(estimatedSurveySpeedMps, 2) + ",";
...
void setup() {
  ...
  Serial.println("AUTONOMY V5.1.3: FOUR-CORNER SURVEY + RELIABLE CONNECTOR STOP");
  Serial.println("======================================");
