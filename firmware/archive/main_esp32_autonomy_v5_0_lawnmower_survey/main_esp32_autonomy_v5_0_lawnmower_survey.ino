...
enum class SurveyPointSlot : uint8_t { NONE, A, B, C };
...
// ============================================================
// V5.0 lawnmower survey controller
// Three averaged GPS points define the rectangle:
//   A = first corner / start of first strip
//   B = end of first strip
//   C = any point on the opposite side of the rectangle
// A->B defines strip direction. The perpendicular projection of C defines width.
// ============================================================
...
float surveyRequestedSpacingM = 3.0f;
...
uint32_t surveyPhotoIntervalMs = 1300;
...
constexpr float SURVEY_MIN_SPACING_M = 2.5f;
constexpr float SURVEY_MAX_SPACING_M = 12.0f;
constexpr uint32_t SURVEY_MIN_PHOTO_INTERVAL_MS = 950;
constexpr uint32_t SURVEY_MAX_PHOTO_INTERVAL_MS = 4000;
constexpr float SURVEY_START_NEAR_M = 5.0f;
constexpr float SURVEY_START_REACHED_M = 2.5f;
constexpr float SURVEY_ENDPOINT_RADIUS_M = 3.0f;
constexpr float SURVEY_CONNECTOR_RADIUS_M = 1.4f;
constexpr float SURVEY_LOOKAHEAD_M = 3.2f;
constexpr float CONNECTOR_LOOKAHEAD_M = 1.8f;
...
void loadSurveyConfigFromNvs() {
  ...
  surveyRequestedSpacingM = constrain(preferences.getFloat("surveySpace", 3.0f), SURVEY_MIN_SPACING_M, SURVEY_MAX_SPACING_M);
  surveyPhotoIntervalMs = constrain(preferences.getUInt("surveyPhoto", 1300), SURVEY_MIN_PHOTO_INTERVAL_MS, SURVEY_MAX_PHOTO_INTERVAL_MS);
}

void clearSurveyArea() {
  surveyASet = surveyBSet = surveyCSet = false;
...
void writeMotors(int leftUs, int rightUs) {
  if (rightUs < ESC_NEUTRAL_US) rightUs += rightForwardTrimUs;
...
void startSurveyPointAveraging(SurveyPointSlot slot) {
  ...
  lastSystemEvent = slot == SurveyPointSlot::A ? "AVERAGING_A" : slot == SurveyPointSlot::B ? "AVERAGING_B" : "AVERAGING_C";
}
...
bool buildSurveyRoute() {
  if (!surveyASet || !surveyBSet || !surveyCSet) {
    lastSystemEvent = "BUILD_BLOCKED_SAVE_A_B_C";
    return false;
  }
  LocalPoint b = geoToLocal(surveyA, surveyB);
  LocalPoint c = geoToLocal(surveyA, surveyC);
  float length = localLength(b);
  if (length < SURVEY_MIN_LENGTH_M || length > SURVEY_MAX_LENGTH_M) {
    lastSystemEvent = "BUILD_BAD_AB_LENGTH";
    return false;
  }
  float ux = b.x / length;
  float uy = b.y / length;
  float nx = -uy;
  float ny = ux;
  float signedWidth = c.x * nx + c.y * ny;
  if (signedWidth < 0.0f) { signedWidth = -signedWidth; nx = -nx; ny = -ny; }
  if (signedWidth < SURVEY_MIN_WIDTH_M || signedWidth > SURVEY_MAX_WIDTH_M) {
    lastSystemEvent = "BUILD_BAD_WIDTH";
    return false;
  }
  uint8_t laneCount = static_cast<uint8_t>(ceilf(signedWidth / surveyRequestedSpacingM)) + 1;
  if (laneCount < 2) laneCount = 2;
  if (laneCount > MAX_SURVEY_LANES) {
    lastSystemEvent = "BUILD_TOO_MANY_LANES_INCREASE_SPACING";
    return false;
  }
  float actualSpacing = signedWidth / static_cast<float>(laneCount - 1);
  uint8_t pointIndex = 0;
  for (uint8_t lane = 0; lane < laneCount; lane++) {
    float offset = actualSpacing * lane;
    LocalPoint start = {nx * offset, ny * offset};
    LocalPoint end = {b.x + nx * offset, b.y + ny * offset};
    if ((lane & 1U) == 0U) {
      surveyRoute[pointIndex++] = localToGeo(surveyA, start);
      surveyRoute[pointIndex++] = localToGeo(surveyA, end);
    } else {
      surveyRoute[pointIndex++] = localToGeo(surveyA, end);
      surveyRoute[pointIndex++] = localToGeo(surveyA, start);
    }
  ...
  surveyLengthM = length;
  surveyWidthM = signedWidth;
...
bool startSurvey() {
  if (!surveyRouteBuilt || surveyRoutePointCount < 2) { lastSystemEvent = "START_BLOCKED_BUILD_ROUTE"; return false; }
  if (!motorsArmed) { lastSystemEvent = "START_BLOCKED_ARM"; return false; }
  if (!gpsGoodForAuto()) { lastSystemEvent = "START_BLOCKED_GPS"; return false; }
  if (!gyroSignVerified) { lastSystemEvent = "START_BLOCKED_GYRO_SIGN"; return false; }
  if (!headingInitialized || bowLockPending) { lastSystemEvent = "START_BLOCKED_LOCK_BOW"; return false; }
  if (millis() - headingLockedAtMs > BOW_LOCK_MAX_START_AGE_MS) { lastSystemEvent = "START_BLOCKED_BOW_LOCK_OLD"; return false; }
  if (millis() - lastMpuGoodMs > MPU_STALE_TIMEOUT_MS) { lastSystemEvent = "START_BLOCKED_MPU"; return false; }
  if (!cameraOnline || !cameraRemoteReady || !cameraSdReady) { lastSystemEvent = "START_BLOCKED_CAMERA_OR_SD"; return false; }
  if (!missionActive) { lastSystemEvent = "START_BLOCKED_START_PHOTO_MISSION"; return false; }
  if (captureRequestPending) { lastSystemEvent = "START_BLOCKED_CAMERA_BUSY"; return false; }
  float distanceFromA = distanceToGeo(surveyA);
  if (distanceFromA < 0.0f || distanceFromA > AUTO_MAX_DISTANCE_FROM_A_M) { lastSystemEvent = "START_BLOCKED_TOO_FAR_FROM_A"; return false; }
  if (distanceFromA > SURVEY_START_NEAR_M) { lastSystemEvent = "START_BLOCKED_MOVE_NEAR_A"; return false; }
...
bool activeLegReached() {
  ...
  return (activeLegAlongM >= activeLegLengthM - 1.4f && currentTargetDistanceM <= SURVEY_ENDPOINT_RADIUS_M) ||
         activeLegAlongM >= activeLegLengthM + 0.5f;
}
...
void printHelp() {
  Serial.println();
  Serial.println("========== V5 SURVEY COMMANDS ==========");
  Serial.println("M / E              - start/end camera mission");
  Serial.println("ARM / DISARM / STOP");
  Serial.println("A_HERE / B_HERE / C_HERE - average survey corners");
...
body{font-family:Arial;background:#101418;color:#fff;text-align:center;margin:0;padding:10px;-webkit-user-select:none;user-select:none}
.card{background:#1d2730;border-radius:14px;padding:12px;margin:10px auto;max-width:720px}
button,input{font-size:17px;padding:13px;margin:5px;border:0;border-radius:11px}
...
button.hold{touch-action:none}.go{background:#29a36a;color:#fff}.stop{background:#d62828;color:#fff}.manual{background:#74c0fc}.mode{background:#f0b429}.safe{background:#6c757d;color:#fff}
input{width:125px;-webkit-user-select:text;user-select:text}.mono{font-family:monospace;text-align:left;white-space:pre-wrap;font-size:13px}.small{font-size:13px;color:#cbd5e1}
</style></head><body>
<h2>Catamaran V5 Survey</h2>
<div class="card"><button class="safe" onclick="cmd('arm')">ARM</button><button class="safe" onclick="cmd('disarm')">DISARM</button><button class="stop" onclick="cmd('stop')">EMERGENCY STOP</button></div>
<div class="card"><h3>Manual hold</h3><button class="manual hold" data-m="f">FORWARD</button><br><button class="manual hold" data-m="l">LEFT</button><button class="stop" onclick="manualStop()">STOP</button><button class="manual hold" data-m="r">RIGHT</button><br><button class="manual hold" data-m="b">REVERSE</button></div>
...
<div class="card"><h3>1. Record survey rectangle</h3><p class="small">A=start corner, B=end of first strip, C=any point on the opposite side. Keep still for 6 seconds.</p><button class="mode" onclick="cmd('savea')">SAVE A HERE</button><button class="mode" onclick="cmd('saveb')">SAVE B HERE</button><button class="mode" onclick="cmd('savec')">SAVE C HERE</button><br><button class="safe" onclick="cmd('cleararea')">CLEAR AREA</button></div>
<div class="card"><h3>2. Coverage settings</h3><label>Strip spacing m<br><input id="spacing" type="number" step="0.1" min="2.5" max="12" value="3.0"></label><br><label>Photo interval ms<br><input id="photo" type="number" step="50" min="950" max="4000" value="1300"></label><br><button class="mode" onclick="saveConfig()">SAVE SETTINGS</button><button class="go" onclick="cmd('build')">BUILD LAWNMOWER ROUTE</button></div>
<div class="card"><h3>3. Heading and camera</h3><button class="mode" onclick="cmd('levelzero')">SET WATER LEVEL ZERO</button><br><p class="small">Put the boat near A. Point the bow toward B, then lock.</p><button class="go" onclick="cmd('lockbow')">LOCK BOW FOR SURVEY (2 s)</button><button class="safe" onclick="cmd('clearbow')">CLEAR BOW LOCK</button><br><button class="mode" onclick="cmd('missionstart')">START PHOTO MISSION</button><button class="mode" onclick="cmd('capturetest')">TEST PHOTO</button></div>
<div class="card"><h3>4. Run</h3><button class="go" onclick="cmd('survey')">START LAWNMOWER SURVEY</button><button class="mode" onclick="cmd('missionend')">END PHOTO MISSION</button></div>
...
async function saveConfig(){const s=document.getElementById('spacing').value;const p=document.getElementById('photo').value;await fetch('/surveyconfig?spacing='+encodeURIComponent(s)+'&photo='+encodeURIComponent(p));update()}
async function update(){try{const s=await(await fetch('/status',{cache:'no-store'})).json();document.getElementById('spacing').value=s.requestedSpacing;document.getElementById('photo').value=s.photoInterval;document.getElementById('status').textContent=
...
'A:'+s.aSet+' B:'+s.bSet+' C:'+s.cSet+' ROUTE:'+s.routeBuilt+'\n'+
'AREA: '+s.length+' x '+s.width+' m  LANES:'+s.lanes+' ACTUAL SPACING:'+s.actualSpacing+' m\n'+
...
void handleControlStatus() {
  ...
  String avgState = surveyPointAveragingSlot == SurveyPointSlot::NONE ? "NO" : surveyPointAveragingSlot == SurveyPointSlot::A ? "A" : surveyPointAveragingSlot == SurveyPointSlot::B ? "B" : "C";
  ...
  json += "\"aSet\":\"" + String(surveyASet ? "YES" : "NO") + "\",\"bSet\":\"" + String(surveyBSet ? "YES" : "NO") + "\",\"cSet\":\"" + String(surveyCSet ? "YES" : "NO") + "\",";
  json += "\"routeBuilt\":\"" + String(surveyRouteBuilt ? "YES" : "NO") + "\",\"length\":" + jsonNumber(surveyLengthM, 1) + ",\"width\":" + jsonNumber(surveyWidthM, 1) + ",";
...
void setup() {
  ...
  Serial.println("AUTONOMY V5.0: LAWNMOWER SURVEY + GYRO HEADING + CONTINUOUS PHOTOS");
  Serial.println("======================================");
