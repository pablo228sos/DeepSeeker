...
// ============================================================
// Motor layer + gyro-heading single-waypoint autonomous controller
// ============================================================
...
// Heading controller uses the manually initialized bow heading and gyro yaw.
// GPS course is never allowed to directly command a sharp turn.
constexpr float AUTO_HEADING_DEADBAND_DEG = 5.0f;
constexpr float AUTO_HEADING_KP_US_PER_DEG = 0.55f;
constexpr float AUTO_YAW_DAMPING_US_PER_DEG_PER_SEC = 0.90f;
constexpr float AUTO_MAX_CORRECTION_US = 38.0f;
constexpr float AUTO_STEERING_SLEW_US_PER_UPDATE = 5.0f;
constexpr int AUTO_WEAKEST_FORWARD_US = 1450;

// Very slow GPS-course correction for long-term gyro drift. It is active only
// during straight, steady motion and cannot produce a sudden steering command.
constexpr float HEADING_GPS_MIN_SPEED_KMPH = 1.10f;
constexpr float HEADING_GPS_MAX_YAW_RATE_DPS = 5.0f;
constexpr float HEADING_GPS_MAX_COURSE_DELTA_DEG = 28.0f;
constexpr float HEADING_GPS_CORRECTION_GAIN = 0.035f;
constexpr float HEADING_GPS_MAX_STEP_DEG = 0.8f;
constexpr uint32_t HEADING_GPS_CORRECTION_INTERVAL_MS = 800;
...
constexpr uint32_t BOW_LOCK_MAX_START_AGE_MS = 120000;
...
void requestGyroSignRelearn() {
  ...
  Serial.println("Gyro sign relearn: hold manual LEFT or RIGHT for at least 1 second");
}
...
void setTarget(double latitude, double longitude) {
  ...
  // The absolute bow reference belongs to the previous target. Require a new
  // physical bow alignment whenever the target changes.
  headingInitialized = false;
  bowLockPending = false;
  headingEstimateDeg = 0.0f;
  currentHeadingErrorDeg = 0.0;
  lastSystemEvent = "TARGET_SET_LOCK_BOW_REQUIRED";
...
bool startSingleWaypointAuto() {
  if (!motorsArmed) {
    ...
    lastSystemEvent = "AUTO_BLOCKED_ARM";
    return false;
  }
  if (!targetSet) {
    Serial.println("AUTO BLOCKED: set TARGET first");
    lastSystemEvent = "AUTO_BLOCKED_NO_TARGET";
    return false;
  }
  if (!gpsGoodForAuto()) {
    Serial.println("AUTO BLOCKED: GPS quality is insufficient");
    lastSystemEvent = "AUTO_BLOCKED_GPS";
    return false;
  }
  if (!gyroSignVerified) {
    Serial.println("AUTO BLOCKED: hold manual LEFT or RIGHT for 1 second to verify gyro sign");
    lastSystemEvent = "AUTO_BLOCKED_GYRO_SIGN";
    return false;
  }
  if (!headingInitialized || bowLockPending) {
    Serial.println("AUTO BLOCKED: point bow at target and press LOCK BOW TO TARGET");
    lastSystemEvent = "AUTO_BLOCKED_BOW_NOT_LOCKED";
    return false;
  }
  if (millis() - headingLockedAtMs > BOW_LOCK_MAX_START_AGE_MS) {
    Serial.println("AUTO BLOCKED: bow lock is old; lock bow to target again");
    lastSystemEvent = "AUTO_BLOCKED_BOW_LOCK_OLD";
    return false;
  }
  if (millis() - lastMpuGoodMs > MPU_STALE_TIMEOUT_MS) {
    Serial.println("AUTO BLOCKED: MPU data is stale");
    lastSystemEvent = "AUTO_BLOCKED_MPU";
    return false;
  }
  if (!cameraOnline) {
    Serial.println("AUTO BLOCKED: camera is offline");
    lastSystemEvent = "AUTO_BLOCKED_CAMERA";
    return false;
  }
  if (!missionActive) {
    Serial.println("AUTO BLOCKED: start camera mission first");
    lastSystemEvent = "AUTO_BLOCKED_MISSION";
    return false;
  }
  ...
  if (
    ...
    Serial.print("AUTO BLOCKED: target must be 8-35 m away, current: ");
    Serial.print(currentTargetDistanceM, 1);
    Serial.println(" m");
    lastSystemEvent = "AUTO_BLOCKED_DISTANCE";
    return false;
  }

  currentHeadingErrorDeg = normalizeHeadingError(
    static_cast<float>(currentTargetBearingDeg) - headingEstimateDeg
  );
  ...
  autoStateStartedMs = millis();
  autoMessage = "GYRO_HEADING_ACTIVE";
  lastSystemEvent = "AUTO_GYRO_HEADING_START";
  Serial.println("AUTO SINGLE WAYPOINT STARTED: gyro bow heading active");
  return true;
}
...
bool startBowLockCalibration() {
  if (!targetSet) {
    Serial.println("BOW LOCK BLOCKED: set target first");
    lastSystemEvent = "BOW_LOCK_BLOCKED_NO_TARGET";
    return false;
  }
  if (!gpsGoodForAuto()) {
    Serial.println("BOW LOCK BLOCKED: need good GPS fix");
    lastSystemEvent = "BOW_LOCK_BLOCKED_GPS";
    return false;
  }
  if (!gyroSignVerified) {
    Serial.println("BOW LOCK BLOCKED: hold manual LEFT or RIGHT for 1 second first");
    lastSystemEvent = "BOW_LOCK_BLOCKED_GYRO_SIGN";
    return false;
  }
  if (millis() - lastMpuGoodMs > MPU_STALE_TIMEOUT_MS) {
    Serial.println("BOW LOCK BLOCKED: MPU data is stale");
    lastSystemEvent = "BOW_LOCK_BLOCKED_MPU";
    return false;
  }
  ...
  lastSystemEvent = "BOW_LOCK_HOLD_STEADY_2S";

  Serial.println("BOW LOCK STARTED: point bow at target and hold steady for 2 seconds");
  return true;
}
...
void clearBowLock() {
  ...
  Serial.println("Bow heading reference cleared");
}
...
void updateBowLockCalibration() {
  ...
  uint32_t nowMs = millis();
  if (!gpsGoodForAuto() || millis() - lastMpuGoodMs > MPU_STALE_TIMEOUT_MS) {
    bowLockPending = false;
    lastSystemEvent = "BOW_LOCK_FAILED_SENSOR";
    Serial.println("BOW LOCK FAILED: GPS or MPU unavailable");
    return;
  }

  if (
    bowLockLastSampleMs == 0 ||
    nowMs - bowLockLastSampleMs >= BOW_LOCK_SAMPLE_INTERVAL_MS
  ) {
  ...
  if (bowLockSampleCount < 20) {
    bowLockPending = false;
    lastSystemEvent = "BOW_LOCK_FAILED_SAMPLES";
    Serial.println("BOW LOCK FAILED: not enough samples");
    return;
  }

  float averageGyroZ = static_cast<float>(
    bowLockGyroSum / static_cast<double>(bowLockSampleCount)
  );
  float averageAbsGyroZ = static_cast<float>(
    bowLockAbsGyroSum / static_cast<double>(bowLockSampleCount)
  );

  if (
    fabsf(averageGyroZ) > BOW_LOCK_MAX_AVERAGE_GYRO_DPS ||
    averageAbsGyroZ > BOW_LOCK_MAX_AVERAGE_ABS_GYRO_DPS
  ) {
    bowLockPending = false;
    lastSystemEvent = "BOW_LOCK_FAILED_MOVING";
    Serial.print("BOW LOCK FAILED: boat was turning, avg/abs gyro Z=");
    Serial.print(averageGyroZ, 2);
    Serial.print(" / ");
    Serial.println(averageAbsGyroZ, 2);
    return;
  }

  // Refine the Z gyro bias at the exact moment the absolute heading is set.
  gyroOffsetZ += averageGyroZ;
  currentGyroZ -= averageGyroZ;

  float averageBearingDeg = atan2f(
    static_cast<float>(bowLockBearingSinSum),
    static_cast<float>(bowLockBearingCosSum)
  ) * 180.0f / PI;
  averageBearingDeg = normalizeHeading360(averageBearingDeg);

  headingEstimateDeg = averageBearingDeg;
  ...
  currentHeadingErrorDeg = 0.0;
  bowLockPending = false;
  lastSystemEvent = "BOW_LOCKED_TO_TARGET";

  Serial.print("BOW LOCK OK: heading=");
  Serial.print(headingEstimateDeg, 1);
  Serial.print(" deg, refined gyro Z by ");
  Serial.println(averageGyroZ, 3);
}

void updateHeadingGpsCorrection() {
  if (!headingInitialized || autoState != AutoState::NAVIGATE) return;

  uint32_t nowMs = millis();
  if (
    nowMs - lastGpsHeadingCorrectionMs <
      HEADING_GPS_CORRECTION_INTERVAL_MS
  ) {
    return;
  }
  ...
  // Compare pre-trim steering commands. A correct straight command can show a
  // 30-40 us L/R difference after the stronger right motor is compensated.
  int rightPreTrimApprox = currentRightUs;
  if (currentRightUs < ESC_NEUTRAL_US) {
    rightPreTrimApprox -= rightForwardTrimUs;
  }
  if (abs(currentLeftUs - rightPreTrimApprox) > 18) return;
  if (currentTargetDistanceM > 0.0 && currentTargetDistanceM < 6.0) return;

  float courseDelta = normalizeHeadingError(
    static_cast<float>(gps.course.deg()) - headingEstimateDeg
  );

  // Ignore large disagreement. It can be side-slip, current, a delayed GPS
  // course update, or a turn. Such a reading must never steer the boat.
  if (fabsf(courseDelta) > HEADING_GPS_MAX_COURSE_DELTA_DEG) return;

  float correctionStep = constrain(
    courseDelta * HEADING_GPS_CORRECTION_GAIN,
    -HEADING_GPS_MAX_STEP_DEG,
    HEADING_GPS_MAX_STEP_DEG
  );

  headingEstimateDeg = normalizeHeading360(
    headingEstimateDeg + correctionStep
  );
...
void driveTowardTarget() {
  if (!headingInitialized) {
    stopAutonomy("HEADING_REFERENCE_LOST", true);
    return;
  }
  ...
  if (fabsf(static_cast<float>(currentHeadingErrorDeg)) < AUTO_HEADING_DEADBAND_DEG) {
    currentHeadingErrorDeg = 0.0f;
  }

  float absoluteError = fabsf(static_cast<float>(currentHeadingErrorDeg));
  float compassYawRate = static_cast<float>(gyroCompassSign) * currentGyroZ;

  // P controls the requested turn. Direct gyro yaw-rate damping prevents the
  // boat from continuing to rotate after the bow has crossed the target line.
  float targetCorrection =
    AUTO_HEADING_KP_US_PER_DEG *
      static_cast<float>(currentHeadingErrorDeg) -
    AUTO_YAW_DAMPING_US_PER_DEG_PER_SEC * compassYawRate;

  float dynamicMaxCorrection = 12.0f;
  if (absoluteError >= 20.0f) dynamicMaxCorrection = 20.0f;
  if (absoluteError >= 50.0f) dynamicMaxCorrection = 30.0f;
  if (absoluteError >= 90.0f) dynamicMaxCorrection = AUTO_MAX_CORRECTION_US;
  ...
  // Wide forward arcs only. Even at 180 degrees both motors stay forward.
  if (absoluteError > 30.0f) baseUs = max(baseUs, 1385);
  if (absoluteError > 65.0f) baseUs = max(baseUs, 1405);
  if (absoluteError > 115.0f) baseUs = max(baseUs, 1420);

  // Positive error = target is to the right/clockwise.
  ...
  // Reserve room for the positive right-motor trim before writeMotors adds it.
...
void updateAutopilot() {
  ...
  if (nowMs - lastMpuGoodMs > MPU_STALE_TIMEOUT_MS) {
    stopAutonomy("MPU_DATA_LOST", true);
    return;
  }
  ...
  switch (autoState) {
    case AutoState::NAVIGATE:
      if (currentTargetDistanceM <= AUTO_ARRIVAL_RADIUS_M) {
        ...
        autoMessage = "ARRIVED";
        lastSystemEvent = "ARRIVED_GYRO_HEADING";
        break;
      }
...
void printNavigationStatus() {
  ...
  Serial.print("Bow heading locked: ");
  Serial.println(headingInitialized ? "YES" : (bowLockPending ? "PENDING" : "NO"));
  Serial.print("Heading estimate: ");
  Serial.print(headingEstimateDeg, 1);
  Serial.println(" deg");
  Serial.print("GPS drift correction total: ");
  Serial.print(gpsHeadingCorrectionTotalDeg, 1);
  Serial.println(" deg");
  if (targetSet) {
    Serial.print("Target: ");
    Serial.print(targetLatitude, 7);
...
void printHelp() {
  ...
  Serial.println("LOCKBOW           - point bow at target and lock heading for 2 s");
  Serial.println("CLEARBOW          - clear absolute bow heading reference");
  Serial.println("GYROSIGNRESET     - relearn gyro sign using manual LEFT/RIGHT");
  Serial.println("AUTO              - gyro-heading navigate, stop and capture");
...
void processUsbCommand(String command) {
  ...
  } else if (upper == "LOCKBOW") {
    startBowLockCalibration();
  } else if (upper == "CLEARBOW") {
    clearBowLock();
  } else if (upper == "GYROSIGNRESET") {
    requestGyroSignRelearn();
  } else if (upper == "AUTO") {
...
void manualMotorCommand(const String& command) {
  ...
  if (command == "0") {
    ...
    if (gyroTurnLearning && millis() - gyroTurnLearningStartedMs < 800) {
      gyroTurnLearning = false;
      gyroTurnExpectedDirection = 0;
      gyroTurnSampleSum = 0.0;
      gyroTurnSampleCount = 0;
      lastSystemEvent = "GYRO_SIGN_TEST_TOO_SHORT";
    }
    return;
  }
  ...
  // A deliberate manual turn is also the one-time gyro sign verification.
  if (command == "l") beginGyroTurnLearning(-1);
  if (command == "r") beginGyroTurnLearning(+1);

  // Conservative recovery/manual values.
...
void updateManualDriveTimeout() {
  if (
    ...
    if (gyroTurnLearning) {
      ...
      gyroTurnSampleSum = 0.0;
      gyroTurnSampleCount = 0;
      lastSystemEvent = "GYRO_SIGN_TEST_TIMEOUT";
    }
...
<p>First use: hold LEFT or RIGHT for at least 1 second to verify gyro sign.</p>
...
<div class="card">
<h3>Target and heading setup</h3>
...
<button class="mode" onclick="setTarget()">SET COORDINATES</button><br>
<p>After moving the boat to the start, point its bow exactly at the target.</p>
<button class="go" onclick="cmd('lockbow')">LOCK BOW TO TARGET (2 s)</button><br>
<button class="safe" onclick="cmd('clearbow')">CLEAR BOW LOCK</button>
<button class="safe" onclick="cmd('gyrosignreset')">RELEARN GYRO SIGN</button>
</div>
<div class="card">
<h3>Mission</h3>
...
<button class="go" onclick="cmd('auto')">START GYRO-HEADING AUTO</button>
...
async function update(){
 try{
  ...
  document.getElementById('status').textContent=
   ...
   'LAT: '+s.lat+'\nLON: '+s.lon+'\n'+
   'SPEED: '+s.speed+' km/h  GPS COURSE: '+s.course+' deg\n'+
   'HEADING EST: '+s.heading+' deg  BOW LOCK: '+s.bowLock+'\n'+
   'GYRO SIGN: '+s.gyroSign+'  VERIFIED: '+s.gyroSignVerified+'  GPS CORR: '+s.gpsHeadingCorr+' deg\n'+
...
void handleControlStatus() {
  ...
  json += "\"heading\":" + jsonNumber(headingInitialized ? headingEstimateDeg : 0.0, 1) + ",";
  json += "\"bowLock\":\"" + bowLockState + "\",";
  json += "\"gyroSign\":" + String(static_cast<int>(gyroCompassSign)) + ",";
  json += "\"gyroSignVerified\":\"" + String(gyroSignVerified ? "YES" : "NO") + "\",";
  json += "\"gpsHeadingCorr\":" + jsonNumber(gpsHeadingCorrectionTotalDeg, 1) + ",";
...
void printSystemState() {
  ...
  if (headingInitialized) {
    Serial.print(headingEstimateDeg, 1);
  } else if (bowLockPending) {
    Serial.print("LOCKING");
  } else {
    Serial.print("UNLOCKED");
  }
  Serial.print(" GYRO_SIGN:");
  Serial.print(static_cast<int>(gyroCompassSign));
  Serial.print(gyroSignVerified ? "V" : "?");
...
void setup() {
  ...
  Serial.println("AUTONOMY V4.5: MANUAL BOW LOCK + MPU GYRO HEADING + GPS POSITION");
  Serial.println("======================================");
