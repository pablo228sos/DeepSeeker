...
constexpr float AUTO_MIN_COURSE_SPEED_KMPH = 0.40f;
...
constexpr uint32_t AUTO_SETTLE_FORCE_CAPTURE_MS = 5000;
...
constexpr float AUTO_HEADING_DEADBAND_DEG = 4.0f;
constexpr float AUTO_COURSE_FILTER_GAIN = 0.30f;
constexpr float AUTO_HEADING_KP_US_PER_DEG = 1.15f;
constexpr float AUTO_HEADING_KD_US_PER_DEG_PER_SEC = 0.45f;
constexpr float AUTO_MAX_CORRECTION_US = 100.0f;
constexpr float AUTO_STEERING_SLEW_US_PER_UPDATE = 8.0f;
...
void driveTowardTarget() {
  if (!gpsCourseReady()) {
    writeMotors(AUTO_SLOW_US, AUTO_SLOW_US);
    return;
  }

  updateFilteredGpsCourse();

  const float currentCourse =
    filteredCourseValid
      ? filteredCourseDeg
      : static_cast<float>(gps.course.deg());
  ...
  uint32_t nowMs = millis();
  float dtSeconds =
    lastSteeringUpdateMs == 0
      ? 0.1f
      : (nowMs - lastSteeringUpdateMs) / 1000.0f;

  dtSeconds = constrain(dtSeconds, 0.05f, 0.5f);
  lastSteeringUpdateMs = nowMs;

  float errorDelta = normalizeHeadingError(
    static_cast<float>(currentHeadingErrorDeg) -
    previousHeadingErrorDeg
  );

  float rawErrorRate = errorDelta / dtSeconds;
  filteredHeadingErrorRateDegPerSec =
    0.75f * filteredHeadingErrorRateDegPerSec +
    0.25f * rawErrorRate;
  ...
  float targetCorrection =
    AUTO_HEADING_KP_US_PER_DEG *
      static_cast<float>(currentHeadingErrorDeg) +
    AUTO_HEADING_KD_US_PER_DEG_PER_SEC *
      filteredHeadingErrorRateDegPerSec;

  float dynamicMaxCorrection = AUTO_MAX_CORRECTION_US;
  float absoluteError = fabsf(static_cast<float>(currentHeadingErrorDeg));

  if (absoluteError < 20.0f) {
    dynamicMaxCorrection = 45.0f;
  } else if (absoluteError < 45.0f) {
    dynamicMaxCorrection = 70.0f;
  }
  ...
  float yawRateMagnitude = fabsf(currentGyroZ);
  if (yawRateMagnitude > 15.0f) {
    float damping = constrain(
      1.0f - (yawRateMagnitude - 15.0f) / 70.0f,
      0.40f,
      1.0f
    );
    targetCorrection *= damping;
  }
  ...
  if (absoluteError > 25.0f) {
    baseUs = max(baseUs, 1380);
  }
  if (absoluteError > 50.0f) {
    baseUs = max(baseUs, 1420);
  }
  if (absoluteError > 85.0f) {
    baseUs = max(baseUs, 1450);
  }
  if (yawRateMagnitude > 25.0f) {
    baseUs = max(baseUs, 1420);
  }
  ...
  switch (autoState) {
    case AutoState::ACQUIRE_COURSE:
      ...
      if (gpsCourseReady()) {
        ...
        autoStateStartedMs = nowMs;
        autoMessage = "COURSE_OK";
        lastSystemEvent = "AUTO_NAVIGATE";
        break;
      }
...
<style>
body{font-family:Arial;background:#101418;color:#fff;text-align:center;margin:0;padding:12px}
...
button{min-width:120px}.go{background:#29a36a;color:#fff}.stop{background:#d62828;color:#fff}
.manual{background:#74c0fc}.mode{background:#f0b429}.safe{background:#6c757d;color:#fff}
input{width:260px}.mono{font-family:monospace;text-align:left;white-space:pre-wrap}
</style>
</head>
<body>
...
<script>
let timer=null;
async function cmd(c){await fetch('/cmd?c='+encodeURIComponent(c));setTimeout(update,150)}
async function sendManual(m){try{await fetch('/manual?m='+m)}catch(e){}}
function manualStart(m){manualStop(false);sendManual(m);timer=setInterval(()=>sendManual(m),250)}
...
document.querySelectorAll('.hold').forEach(b=>{
 b.addEventListener('pointerdown',e=>{e.preventDefault();manualStart(b.dataset.m)});
 b.addEventListener('pointerup',e=>{e.preventDefault();manualStop()});
 b.addEventListener('pointercancel',manualStop);
 b.addEventListener('pointerleave',e=>{if(e.buttons)manualStop()});
});
...
async function update(){
 try{
  ...
  document.getElementById('status').textContent=
   ...
   'DIST: '+s.distance+' m  ERROR: '+s.headingError+' deg\n'+
...
void setup() {
  ...
  Serial.println("AUTONOMY V4.2: SMOOTH GPS NAV + MOTOR TRIM + CAPTURE FALLBACK");
  Serial.println("======================================");
